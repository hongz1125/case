...
Player.prototype.message = function() {
    ...
    if (this.distanceValue % 1000 >= 1000 - this.speed) {
        this.speed += 0.2;
    }
    if (this.distanceValue % 1500 >= 1500 - this.speed) {
        if (this.anim.speed < 10) {
            this.anim.speed++;
            this.anim.goframe(0);
        }
    }
...
Player.prototype.colDetection = function(objList) {
    ...
    for (var i = 0; i < objList.length; i++) {
        if (this.y - this.height / 2 + this.obb[3] < objList[i].y - objList[i].height / 2 + objList[i].obb[1] + 15) {
            if (eG.OBBvsOBB(this.testObb(), objList[i].testObb())) {
                flag = true;
            }
        }
    }
